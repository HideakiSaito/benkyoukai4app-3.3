...
ActionDispatch::Reloader.to_prepare do
  ...
  RedmineExtensions::EasySettingPresenter.boolean_keys << :easy_gantt_show_holidays << :easy_gantt_show_project_progress << :easy_gantt_show_lowest_progress_tasks
end
...
Redmine::MenuManager.map :project_menu do |menu|
  menu.push(:easy_gantt, { controller: 'easy_gantt', action: 'issues' },
    param: :project_id,
...
ActiveSupport.on_load(:easyproject, yield: true) do
  ...
  if easy_extensions
    Redmine::MenuManager.map :projects_easy_page_layout_service_box do |menu|
      menu.push(:project_easy_gantt, :issues_easy_gantt_path,
        caption: :label_easy_gantt,
    ...
    Redmine::MenuManager.map :easy_quick_top_menu do |menu|
      menu.push(:project_easy_gantt, { controller: 'easy_gantt', action: 'issues', id: nil, set_filter: 0 },
        caption: :label_easy_gantt,
        parent: :projects,
        html: { class: 'icon icon-stats' },
        if: proc { User.current.allowed_to_globally?(:view_global_easy_gantt) })

      menu.push(:issues_easy_gantt, { controller: 'easy_gantt', action: 'issues', id: nil, set_filter: 0 },
        caption: :label_easy_gantt,
        parent: :issues,
...
Redmine::AccessControl.map do |map|
  map.project_module :easy_gantt do |pmap|
    ...
    pmap.permission(:view_easy_gantt, {
      ...
      easy_gantt_resources: [:index, :project_data, :users_sums]
    }, read: true, require: :member)

    # Edit project level
    pmap.permission(:edit_easy_gantt, {
      easy_gantt: [:change_issue_relation_delay, :reschedule_project],
      easy_gantt_resources: [:index, :project_data, :bulk_update_or_create, :users_sums]
    }, require: :member)
    ...
    pmap.permission(:view_global_easy_gantt, {
      easy_gantt: [:index, :issues, :projects],
      easy_gantt_pro: [:lowest_progress_tasks],
      easy_gantt_resources: [:index, :project_data, :global_data]
    }, global: true, read: true, require: :loggedin)

    # Edit global level
    pmap.permission(:edit_global_easy_gantt, {
      easy_gantt_resources: [:index, :project_data, :global_data, :bulk_update_or_create, :users_sums]
    ...
    pmap.permission(:edit_personal_easy_gantt, {
      easy_gantt_resources: [:global_data, :users_sums, :bulk_update_or_create]
    }, global: true, require: :loggedin)
  end
end
